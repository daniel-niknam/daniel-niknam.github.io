...
echo "$title.\n$description.\nBy $author.\n$content\n$credit" \
  ...
  | piper \
    ...
    --output_file assets/audio/posts/#{post_file_without_ext}.wav
BASH
...
system(<<-BASH)
ffmpeg -y \
  -i assets/audio/posts/#{post_file_without_ext}.wav \
  ...
  -c:a libmp3lame \
  -q:a 4 \
  assets/audio/posts/#{post_file_without_ext}-with-music.mp3
BASH
...
system(<<-BASH)
ffmpeg -y \
  -i assets/audio/posts/#{post_file_without_ext}-with-music.mp3 \
  -vn -ar 44100 -ac 2 -b:a 96k \
  assets/audio/posts/#{post_file_without_ext}.mp3
BASH
...
system(<<-BASH)
rm assets/audio/posts/#{post_file_without_ext}-with-music.mp3 \
  && rm assets/audio/posts/#{post_file_without_ext}.wav
BASH
